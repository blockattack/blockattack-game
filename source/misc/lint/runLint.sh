...
>> "${TMPFILE}"
export LANG=C
find ./source/code/ -name "*.cpp" -not -path "./source/code/Libs/*" | xargs --no-run-if-empty grep --with-filename --line-number -aRnP '[\x80-\xFF]' >> ${TMPFILE}  || true
find ./source/code/ -name "*.h*" -not -path "./source/code/Libs/*" | xargs --no-run-if-empty grep --with-filename --line-number -aRnP '[\x80-\xFF]' >> ${TMPFILE}  || true

if [ -s "$TMPFILE" ]
then
echo "In the following places the lines contain non-ASCII characters."
cat "$TMPFILE"
echo ""
echo "Validation failed"
rm "$TMPFILE"
